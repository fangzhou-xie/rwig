...
// #include "utils.hpp"                // using the utility functions
...
private:
  ...
  void compute_impl() {
    ...
    // barycenter class init, disable BC verbose
    Barycenter bc(_S, true, _maxiter, _zerotol, false);
    bc.update_C(_C);
    bc.update_reg(_reg);
    // bc.update_A(A);
    ...
    for (int e = 0; e < _E; ++e) { // START: epochs
      ...
      for (int bi = 0; bi < batches; ++bi) { // START: one batch
        ...
        for (int bm = 0; bm < _B; ++bm) { // START: one doc in batch
          ...
          if ((_verbose != 0) && (cnt % _verbose) == 0) {
            cpp11::message(
              "Epoch: %i/%i, batch: %i/%i, doc: %i/%i, iter: %.i, avg speed: %.2f sec\n",
              e+1, _E,
              bi+1, batches,
              m+1, _M,
              bc.iter, // bc._err,
...
public:
  ...
  WassersteinDictionaryLearning(
    ...
    const int rng_seed = 123, const int verbose = false
  ) {
    _B = batch_size;
    _E = epochs;
